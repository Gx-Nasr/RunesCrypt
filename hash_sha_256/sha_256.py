...
def binary_blocks(password):
    result = ""
    num_ord = 0
    b_base = "01"
    for c in password:
        num_ord = ord(c)
        binary_number = convert_base(num_ord, "01")
    ...
    len_number = len(password)*8
    binary_len = convert_base(len_number, "01")
...
def sha_256(password):
    blocks = binary_blocks(password)
    for i in range(16, 64):
        new_w = w_generater(blocks, i)
        blocks.append(new_w)
    ...
    for t in range(0, 64):
        b_sigma1 = int(big_sigma1(convert_base(e, "01")), 2)
        ch_1 = int(ch(e,f ,g), 2)
        Wt = int(blocks[t], 2)
        Kt = int(K[t], 16)
    
        T1 = (h + b_sigma1 + ch_1 + Kt + Wt) % (2**32)

        m = int(maj(a, b, c), 2)
        b_sigma0 = int(big_sigma0(convert_base(a, "01")), 2)
